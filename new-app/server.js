...
app.use(session({
  ...
  cookie: { maxAge: 10 * 60 * 1000 } // 30 minutes
}));
...
function requireLogin(req, res, next) {
  if (!req.session.user) {
    // For AJAX/API requests, send 401
...
app.post('/api/employees', requireLogin, async (req, res) => {
  try {
    ...
    const mailOptions = {
      from: '"PathraVahak" <r363523@gmail.com>',
      to: email,
      subject: 'Welcome to PathraVahak!',
      html: `
        <h2>Dear ${empname},</h2>
        <p>You have been added as an employee in PathraVahak.</p>
        <p>Your assigned area: <b>${assignedarea}</b></p>
        <p>Status: <b>${empstatus}</b></p>
    ...
    transporter.sendMail(mailOptions)
      ...
      .catch(emailErr => {
        console.error('Failed to send employee email:', emailErr);
      });
